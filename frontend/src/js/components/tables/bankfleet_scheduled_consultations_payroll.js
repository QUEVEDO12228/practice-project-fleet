export async function loadScheduledConsultationsPayroll() {
    ...
    const response = await fetch("/frontend/src/html/components/electronic payroll/tables/bankfleet_scheduled_consultations_payroll.html");
    if (!response.ok) {throw new Error("Error al cargar bankfleet_scheduled_consultations_payroll.html");}TableScheduledConsultationsPayrollContainer.innerHTML = await response.text();
}
export function initScheduledConsultationsPayroll() {
    // ==========================
    // MENÚS SECUNDARIOS
    // ==========================
    const menus = document.querySelectorAll(".bankfleet_scheduled_consultations-table-employee-menu");
    menus.forEach(menu => {const button = menu.querySelector(".bankfleet_scheduled_consultations-table-employee-menu__btn");
    ...
    // MENÚ PRINCIPAL
    // ==========================
    const menusPrimary = document.querySelectorAll(".bankfleet_scheduled_consultations-table-employee-menu-primary");
    menusPrimary.forEach(menu => {const button = menu.querySelector(".bankfleet_scheduled_consultations-table-employee-menu__btn-primary");
