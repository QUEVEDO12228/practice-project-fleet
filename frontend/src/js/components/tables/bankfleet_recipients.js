...
export async function loadRecipientsTable() {
    ...
    const response = await fetch("/frontend/src/html/components/transfers/tables/bankfleet_recipients.html")
    if (!response.ok) {throw new Error("Error al cargar bankfleet_recipients.html");}TableRecipientsContainer.innerHTML = await response.text();
}
export function initTableRecipients() {
    // ==========================
    // MENÚS SECUNDARIOS
    // ==========================
    const menus = document.querySelectorAll(".bankfleet-recipients-table-employee-menu");
    menus.forEach(menu => {const button = menu.querySelector(".bankfleet-recipients-table-employee-menu__btn");
    ...
    // MENÚ PRINCIPAL
    // ==========================
    const menusPrimary = document.querySelectorAll(".bankfleet-recipients-table-employee-menu-primary");
    menusPrimary.forEach(menu => {const button = menu.querySelector(".bankfleet-recipients-table-employee-menu__btn-primary");
