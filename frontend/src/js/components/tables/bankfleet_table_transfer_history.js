...
export async function loadTransferHistoryTable() {
    ...
    const response = await fetch("/frontend/src/html/components/transfers/tables/bankfleet_table_transfer_history.html")
    if (!response.ok) {throw new Error("Error al cargar bankfleet_table_transfer_history.html");}TableTransferHistoryContainer.innerHTML = await response.text();
}
export function initTableTransferHistory() {
    // ==========================
    // MENÚS SECUNDARIOS
    // ==========================
    const menus = document.querySelectorAll(".bankfleet-transactions-table-employee-menu");
    menus.forEach(menu => {const button = menu.querySelector(".bankfleet-transactions-table-employee-menu__btn");
    ...
    // MENÚ PRINCIPAL
    // ==========================
    const menusPrimary = document.querySelectorAll(".bankfleet-transactions-table-employee-menu-primary");
    menusPrimary.forEach(menu => {const button = menu.querySelector(".bankfleet-transactions-table-employee-menu__btn-primary");
