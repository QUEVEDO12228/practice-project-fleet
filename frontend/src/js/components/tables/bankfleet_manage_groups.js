document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("/frontend/src/html/components/electronic payroll/tables/bankfleet_manage_groups.html")
        ...
        .then(data => {TableManageGroupContainer.innerHTML = data;initTableManagementGroups();if (FilterContainer) {
                fetch("/frontend/src/html/components/electronic payroll/filters/bankfleet_filter_payrollpayment1.html")
                    .then(response => {if (!response.ok) {throw new Error("Error al cargar bankfleet_filter_payrollpayment1.html");}return response.text();})
...
function initTableManagementGroups(){
    const menus = document.querySelectorAll(".bankfleet_manage_groups-table-employee-menu");menus.forEach(menu => {const button = menu.querySelector(".bankfleet_manage_groups-table-employee-menu__btn");
    ...
    const menusPrimary = document.querySelectorAll(".bankfleet_manage_groups-table-employee-menu-primary");menusPrimary.forEach(menu => {const button = menu.querySelector(".bankfleet_manage_groups-table-employee-menu__btn-primary");
