document.addEventListener("DOMContentLoaded", function () {
    const TableTransferConfirmationContainer = document.querySelector(".bankfleet-bank-transfer-container");
    if (!TableTransferConfirmationContainer) {console.warn("No se encontró '.bankfleet-bank-transfer-container' en el HTML.");return;}
    fetch("/frontend/src/html/components/transfers/bankfleet_bank_transfer.html")
        .then(response => {if (!response.ok) {throw new Error("Error al cargar bankfleet_bank_transfer.html");}return response.text();})
...
function initTransferKeyEvents() {
    const continueButton = document.querySelector(".bankfleet-transfer-bank__btn-primary");
