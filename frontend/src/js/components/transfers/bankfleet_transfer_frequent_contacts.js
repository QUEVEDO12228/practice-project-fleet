document.addEventListener("DOMContentLoaded", function () {
    const TableTransferConfirmationContainer = document.querySelector(".bankfleet-transfer-frequent-contacts-container");
    if (!TableTransferConfirmationContainer) {console.warn("No se encontró '.bankfleet-transfer-frequent-contacts-container' en el HTML.");return;}
    fetch("/frontend/src/html/components/transfers/bankfleet_transfer_frequent_contacts.html")
        .then(response => {if (!response.ok) {throw new Error("Error al cargar bankfleet_transfer_frequent_contacts.html");}return response.text();})
        .then(data => {TableTransferConfirmationContainer.innerHTML = data;
            initTransferFrequentContacts();})
        .catch(error => {console.error("Error cargando el componente:", error);});
});
function initTransferFrequentContacts() {
    const continueButton = document.querySelector(".bankfleet-frequent-contacts__btn-primary");
