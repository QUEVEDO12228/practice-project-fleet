document.addEventListener("DOMContentLoaded", function () {
    const componentContainer = document.querySelector(
        ".bankfleet-transfer-frequent-contacts-container"
    );
    if (!componentContainer) {
        console.error(
            "No existe .bankfleet-transfer-frequent-contacts-container en el DOM"
        );
        return;
    }
    const componentPath =
...
function initTransferKeyEvents() {
    const continueButton = document.querySelector(
        ".bankfleet-frequent-contacts__btn-primary"
    );
