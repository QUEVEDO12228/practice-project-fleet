document.addEventListener("DOMContentLoaded", function () {
    const componentContainer = document.querySelector(
        ".bankfleet-transfer-between-my-accounts-container"
    );
    if (!componentContainer) {
        console.error(
            "No existe .bankfleet-transfer-between-my-accounts-container en el DOM"
        );
        return;
    }
    const componentPath =
...
function initTransferKeyEvents() {
    const continueButton = document.querySelector(
        ".bankfleet-transfer-own-accounts__btn-primary"
    );
