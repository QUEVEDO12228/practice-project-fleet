document.addEventListener("DOMContentLoaded", function () {
    const TableTransferConfirmationContainer = document.querySelector(".bankfleet-transfer-between-my-accounts-container");
    if (!TableTransferConfirmationContainer) {console.warn("No se encontró '.bankfleet-transfer-between-my-accounts-container' en el HTML.");return;}
    fetch("/frontend/src/html/components/transfers/bankfleet_transfer_between_my_accounts.html")
        .then(response => {if (!response.ok) {throw new Error("Error al cargar bankfleet_transfer_between_my_accounts.html");}return response.text();})
        .then(data => {TableTransferConfirmationContainer.innerHTML = data;
            initTransferBetweenMyCounts();})
        .catch(error => {console.error("Error cargando el componente:", error);});
});
function initTransferBetweenMyCounts() {
    const continueButton = document.querySelector(".bankfleet-transfer-own-accounts__btn-primary");
