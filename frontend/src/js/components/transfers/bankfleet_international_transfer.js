document.addEventListener("DOMContentLoaded", function () {
    const componentContainer = document.querySelector(
        ".bankfleet-international-transfer-container"
    );
    if (!componentContainer) {
        console.error(
            "No existe .bankfleet-international-transfer-container en el DOM"
        );
        return;
    }
    const componentPath =
        "/frontend/src/html/components/transfers/bankfleet_international_transfer.html";
    fetch(componentPath)
        .then(response => {
            if (!response.ok) {
                throw new Error(
        ...
        .catch(error => {
            console.error(
                "❌ Error cargando bankfleet_bank_transfer.html:",
                error
            );
        });
});
function initTransferKeyEvents() {
    const continueButton = document.querySelector(
        ".bankfleet-transfer-international__btn-primary"
    );
