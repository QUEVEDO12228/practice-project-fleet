document.addEventListener("DOMContentLoaded", function () {
    const TableTransferConfirmationContainer = document.querySelector(".bankfleet-transfer-other-accounts-container");
    if (!TableTransferConfirmationContainer) {console.warn("No se encontró '.bankfleet-transfer-other-accounts-container' en el HTML.");return;}
    fetch("/frontend/src/html/components/transfers/bankfleet_transfer_other_accounts.html")
        .then(response => {if (!response.ok) {throw new Error("Error al cargar bankfleet_transfer_other_accounts.html");}return response.text();})
        .then(data => {TableTransferConfirmationContainer.innerHTML = data;
            initTransferOthersAccounts();})
        .catch(error => {console.error("Error cargando el componente:", error);});
});
function initTransferOthersAccounts() {
    const continueButton = document.querySelector(".bankfleet-transfer-other-accounts__btn-primary");
