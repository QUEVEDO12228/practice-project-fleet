document.addEventListener("DOMContentLoaded", function () {
    const componentContainer = document.querySelector(
        ".bankfleet-transfer-other-accounts-container"
    );
    if (!componentContainer) {
        console.error(
            "No existe .bankfleet-transfer-other-accounts-container en el DOM"
        );
        return;
    }
    const componentPath =
    ...
    fetch(componentPath)
        ...
        .catch(error => {
            console.error(
                "❌ Error cargando bankfleet_transfer_other_accounts.html:",
                error
            );
        });
});
function initTransferKeyEvents() {
    const continueButton = document.querySelector(
        ".bankfleet-transfer-other-accounts__btn-primary"
    );
