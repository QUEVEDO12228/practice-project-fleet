document.addEventListener("DOMContentLoaded", function () {
    const TableTransferConfirmationContainer = document.querySelector(".bankfleet-transfer-by-qr-container");
    if (!TableTransferConfirmationContainer) {console.warn("No se encontró '.bankfleet-transfer-by-qr-container' en el HTML.");return;}
    fetch("/frontend/src/html/components/transfers/bankfleet_transfer_by_qr.html")
        .then(response => {if (!response.ok) {throw new Error("Error al cargar bankfleet_transfer_by_qr.html");}return response.text();})
        .then(data => {TableTransferConfirmationContainer.innerHTML = data;
            initTransferByQR();})
        .catch(error => {console.error("Error cargando el componente:", error);});
});
function initTransferByQR() {
    const continueButton = document.querySelector(".bankfleet-transfer-qr__btn-primary");
