document.addEventListener("DOMContentLoaded", function () {
    const componentContainer = document.querySelector(
        ".bankfleet-key-transfer-component-container"
    );
    if (!componentContainer) {
        console.error(
            "No existe .bankfleet-key-transfer-component-container en el DOM"
        );
        return;
    }
    const componentPath =
        "/frontend/src/html/components/transfers/bankfleet_key_transfer.html";
    fetch(componentPath)
        .then(response => {
            if (!response.ok) {
                throw new Error(
        ...
        .catch(error => {
            console.error(
                "❌ Error cargando bankfleet_key_transfer.html:",
                error
            );
        });
});
function initTransferKeyEvents() {
    const continueButton = document.querySelector(
        ".bankfleet-transfer-key__btn-primary"
    );
